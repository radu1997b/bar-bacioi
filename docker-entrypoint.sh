...
if [ ! -f "$INIT_MARKER" ]; then
  ...
  wrangler d1 execute bar-bacioi-menu --local --file=migration-settings.sql 2>/dev/null || true
  wrangler d1 execute bar-bacioi-menu --local --file=migration-weight.sql 2>/dev/null || true
fi
